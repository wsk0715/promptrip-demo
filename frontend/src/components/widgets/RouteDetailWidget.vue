...
const emit = defineEmits(['trip-update', 'edit-mode-change', 'place-click']);
      ...
      <div class="px-1 -mt-2 mb-6">
        ...
        <div class="flex items-center gap-2 mt-4">
          <span class="px-3 py-1 bg-indigo-50 text-indigo-600 text-[10px] font-black rounded-full border border-indigo-100">
            {{ trip.totalDuration }} 소요 예상
          </span>
        </div>
      </div>

      <div class="space-y-8">
        <div v-for="day in trip.plans" :key="day.day" class="space-y-5">
          ...
          <div class="space-y-6 relative ml-3 border-l-2 border-slate-100 pl-7 py-1">
            <div v-for="(item, idx) in day.items" :key="item.contentId" class="relative group">
              <!-- Marker Icon -->
            ...
            <!-- End of Day Add More (Visible only when editing) -->
