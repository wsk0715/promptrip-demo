...
import { Send, Loader2, Wand2, Clock, ArrowRight, Navigation, SlidersHorizontal, X, Sparkles } from 'lucide-vue-next';
...
<template>
  <div class="h-full flex flex-col overflow-hidden">
    <Transition name="fade-slide" mode="out-in">
      ...
      <div v-if="!tripStore.pendingTrip || tripStore.isProcessing" key="input" class="flex-1 flex flex-col gap-6 p-5 overflow-y-auto custom-scrollbar">
      ...
      <div v-else-if="tripStore.pendingTrip" key="result" class="flex-1 flex flex-col overflow-hidden relative">
          ...
          <div class="space-y-8">
            <div v-for="day in tripStore.pendingTrip.plans" :key="day.day" class="space-y-5">
              ...
              <div class="space-y-6 relative ml-3 border-l-2 border-slate-100 pl-7 py-1">
                <div v-for="(item, idx) in day.items" :key="idx" class="relative group">
                  ...
                  >
                    <button 
                      ...
                      class="absolute -top-2 -right-2 w-6 h-6 bg-white border border-slate-200 rounded-full flex items-center justify-center text-slate-400 hover:text-rose-500 hover:border-rose-100 hover:bg-rose-50 transition-all opacity-0 group-hover:opacity-100 shadow-sm z-10"
                    >
                      <X class="w-3.5 h-3.5" />
                    </button>
                ...
                <div class="pt-2 flex justify-center -ml-7">
                  <button class="flex items-center gap-2 px-4 py-2 bg-slate-50 border border-dashed border-slate-200 rounded-2xl text-[10px] font-black text-slate-400 hover:bg-indigo-50 hover:border-indigo-100 hover:text-indigo-600 transition-all">
        ...
        <div class="absolute bottom-0 inset-x-0 bg-white/90 backdrop-blur-md border-t border-slate-100 p-5 flex gap-3 shadow-[0_-10px_20px_rgba(0,0,0,0.02)]">
          <button class="flex-1 py-4 bg-slate-100 text-slate-600 rounded-2xl text-xs font-black hover:bg-slate-200 transition-colors flex items-center justify-center gap-2">
            <SlidersHorizontal class="w-4 h-4" />
            경로 수정하기
          </button>
          <button 
            @click="tripStore.confirmTrip(); emit('trip-update', tripStore.currentTrip)"
            class="flex-1 py-4 bg-indigo-600 text-white rounded-2xl text-xs font-black shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-colors flex items-center justify-center gap-2"
          >
